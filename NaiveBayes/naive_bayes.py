...
def predict_statistic(teks):
    ...
    pattern = r'(?<!\d)\. (?!\d)|(?<=\w)[!?]'
    request_teks = [k.strip() for k in re.split(pattern, teks) if k.strip()]
    ...
    for teks in request_teks:
        ...
        for dataTest in test_data:
            ...
            predicted_arr.append({'class':predicted_class,'prob':f"{probabilitas_gabungan[predicted_class]:.10%}"})

        if(len(teksNew) == len(predicted_arr)):
            panjang = len(teksNew)
            for i in range(panjang):
                result_arr.append({'teks':teksNew[i][0],'label':predicted_arr[i]['class'],'probabilitas':predicted_arr[i]['prob']})
